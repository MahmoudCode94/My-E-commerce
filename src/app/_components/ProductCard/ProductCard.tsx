...
export default function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <Card className="relative flex flex-row gap-5 pb-4 md:pb-1 overflow-hidden transition-all duration-300 bg-white border-transparent shadow-sm group hover:shadow-xl hover:border-slate-200 md:flex-col md:min-h-[440px]">
      <Link href={`/products/${productId}`} className="flex flex-row w-full gap-7 md:flex-col contents">
        ...
        <div className="flex flex-col flex-1 px-4 py-4 md:p-0 md:justify-between">
          <div className="space-y-1 md:px-4 md:pt-2">
            <h3 className="text-sm font-bold leading-tight line-clamp-2 md:line-clamp-1 text-slate-800">{product.title}</h3>
            <p className="text-xs leading-snug line-clamp-2 text-slate-500 md:text-sm">
              {product.description}
            </p>
          ...
          <div className="flex md:hidden items-center gap-2 mt-auto pt-2">
            <button
              ...
              disabled={isAddingToCart}
              className="flex-1 flex items-center justify-center gap-2 bg-slate-950 text-white py-2 rounded-lg text-xs font-bold"
            >
